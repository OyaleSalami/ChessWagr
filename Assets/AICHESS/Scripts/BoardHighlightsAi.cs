...
public class BoardHighlightsAi : MonoBehaviour
{
    public static BoardHighlightsAi Instance { set; get; }
                ...
                if(allowedMoves[i,j])
                {
                    ...
                    else
                    {
                        ...
                        if (BoardManager.Instance.EnPassant[0] == i && BoardManager.Instance.EnPassant[1] == j 
                            && BoardManager.Instance.SelectedChessman.GetType() == typeof(PawnAi))
                            SetTilePurple(i, j);
                        // Highlight Castling move
                        else if (BoardManager.Instance.SelectedChessman.GetType() == typeof(KingAi) 
    ...
    private void HighlightCheckers(int x, int y, ChessmanAi king)
    {
        ChessmanAi[,] Chessmans = BoardManager.Instance.Chessmans;
        ChessmanAi piece = null;
        ...
        if(y - 1 >= 0)
        {
            ...
            if(piece != null && piece.isWhite != king.isWhite &&  piece.GetType() == typeof(KingAi))
            {
                SetTileRed(x, y - 1);
                return;
            }
        ...
        while (y-- > 0)
        {
            ...
            if(piece.GetType() == typeof(RookAi) || piece.GetType() == typeof(QueenAi))
            {
        ...
        if(x + 1 <= 7 && y - 1 >= 0 && king.isWhite)
        {
            ...
            if(piece != null && piece.isWhite != king.isWhite && piece.GetType() == typeof(PawnAi))
            {
                SetTileRed(x + 1, y - 1);
                return;
            }
        ...
        while (x++ < 7 && y-- > 0)
        {
            ...
            // Else if the piece is from opponent team
            if(piece.GetType() == typeof(Bishup) || piece.GetType() == typeof(QueenAi))
            {
