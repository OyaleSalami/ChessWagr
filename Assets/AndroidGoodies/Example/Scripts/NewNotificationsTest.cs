...
#pragma warning disable 0649
		[SerializeField] InputField _channelIdInputField;

		[SerializeField] InputField _groupIdInputField;

		[SerializeField] Text _resultText;

		[SerializeField] Sprite _goodiesSprite;

		[SerializeField] Sprite _bigPictureSprite;
#pragma warning restore 0649
		...
		void Start()
		{
			...
			AGNotificationManager.SetOnScheduledNotificationReceivedListener(id =>
				Debug.Log($"Scheduled notification with id {id} has arrived."));

			RequestPermissionAndroid13();
		}

		static void RequestPermissionAndroid13()
		{
		...
		public void LogNotificationParams()
		{
			...
			{
				var text =
					$"App was opened via notification with parameters: {AGNotificationManager.GetNotificationParameter(ParamKeyId)}, {AGNotificationManager.GetNotificationParameter(ParamKeyDate)}";
		...
		{
			var channel = new NotificationChannel(channelId, channelName, AGNotificationManager.Importance.Max)
			{
				Group = group,
			...
			channel.SetSoundFromRawResource("notification_sound", CreateAudioAttributes());
...
#pragma warning restore 0618
			...
			builder.SetSoundFromRawResource("notification_sound");
		...
		public void CreateDifferentNotifications()
		{
			...
			if (AGDeviceInfo.SDK_INT >= AGDeviceInfo.VersionCodes.KITKAT_WATCH)
			{
				builder = CreateBaseNotificationBuilder()
					...
					.AddAction(Notification.CreateOpenUrlAction(
						"https://github.com/TarasOsiris/android-goodies-docs-PRO/wiki", "notify_icon_small",
						"Open URL"));
				Notify(builder);
			}
