...
public class Board : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            return; // Exit if the mouse is outside the screen
        }
    ...
    {
        // Save the current position of the piece to reset after the function call
        int originalX = cp.currentX, originalY = cp.currentY;
        List<Vector2Int> movesToRemove = new List<Vector2Int>();

        // Iterate through all available moves, simulate them, and check if the king is in check
        for (int i = 0; i < moves.Count; i++)
        {
            int simulatedX = moves[i].x, simulatedY = moves[i].y;

            // Get the king's position for this simulation
            Vector2Int simulatedKingPosition = new Vector2Int(targetKing.currentX, targetKing.currentY);

            // If the current piece is the king, simulate its move
            if (cp.type == ChessPieceType.King)
                simulatedKingPosition = new Vector2Int(simulatedX, simulatedY);

            // Create a simulated board and copy the current state of the board
            ChessPiece[,] simulation = new ChessPiece[BOARD_SIZE, BOARD_SIZE];
            List<ChessPiece> simulatedAttackingPieces = new List<ChessPiece>();

            // Populate the simulation board and collect enemy pieces
            for (int x = 0; x < BOARD_SIZE; x++)
            {
                for (int y = 0; y < BOARD_SIZE; y++)
                {
                    if (HasPiece(chessPieces[x, y]))
                    {
                        ...
                        if (simulation[x, y].team != cp.team)
                            simulatedAttackingPieces.Add(simulation[x, y]);
                    }
                }
            }

            // Simulate the move: remove the piece from its original position and place it in the simulated destination
            simulation[originalX, originalY] = null;
            cp.currentX = simulatedX;
            cp.currentY = simulatedY;
            simulation[simulatedX, simulatedY] = cp;

            // Check if any enemy piece was captured during this simulation
            var capturedPiece = simulatedAttackingPieces.Find(c => c.currentX == simulatedX && c.currentY == simulatedY);
            if (HasPiece(capturedPiece))
                simulatedAttackingPieces.Remove(capturedPiece);

            // Get all the moves for the enemy pieces in the simulated scenario
            List<Vector2Int> simulatedEnemyMoves = new List<Vector2Int>();
            foreach (var enemyPiece in simulatedAttackingPieces)
            {
                var enemyMoves = enemyPiece.GetAvailableMoves(ref simulation, BOARD_SIZE, BOARD_SIZE);
                simulatedEnemyMoves.AddRange(enemyMoves);
            }

            // If the king is in check in the simulated move, mark the move as invalid
            if (ContainsValidMove(ref simulatedEnemyMoves, simulatedKingPosition))
                movesToRemove.Add(moves[i]);

            // Restore the piece's original position after simulation
            cp.currentX = originalX;
            cp.currentY = originalY;
        }

        // Remove the invalid moves from the original list of available moves
    ...
    {
        //StartUP.instance.SoundOptions.clip = StartUP.instance.InGameDanger;
        //StartUP.instance.SoundOptions.Play();
    ...
    public bool CheckForWin(int currentPlayerTeam)
    {
        // Logic to determine if the opposing player's king is captured
        foreach (var piece in chessPieces)
        {
            if (piece != null && piece is King && piece.team != currentPlayerTeam)
            {
                return false; // The king is still on the board, no winner yet
            }
        }
        return true; // No opposing king found, current player wins
    }
